...
interface Message { id: string; type: 'email' | 'call' | 'text'; summary: string; date: string; }
...
async function getClientData(id: string) {
  ...
  const { results: tasks } = await env.DB.prepare(`SELECT * FROM tasks WHERE client_id = ? ORDER BY due_date ASC`).bind(id).all<Task>();
  const { results: messages } = await env.DB.prepare(`SELECT * FROM messages WHERE client_id = ? ORDER BY date DESC`).bind(id).all<Message>();
    ...
    <main className="min-h-screen bg-lumaire-ivory p-8">
      ...
      <div className="flex justify-between items-start mb-8 pb-8 border-b border-lumaire-brown/10">
        ...
        <div className="flex gap-3">
          <button className="px-4 py-2 bg-lumaire-brown text-white text-sm hover:bg-lumaire-wine transition-colors">Contact Client</button>
        </div>
      </div>

      {/* TOP ROW: FINANCIALS, STATUS, AND NOTES */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        {/* NEW: NOTES WIDGET (Spans 2 cols on wide screens) */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          ...
          {messages.length === 0 ? (
            <div className="p-6 bg-white border border-dashed border-lumaire-brown/20 text-center opacity-50">No messages logged yet.</div>
          ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-lumaire-tan/20">
              ...
              {tasks.length === 0 && <div className="p-6 text-center opacity-50">No tasks.</div>}
            </div>
        </div>
      </div>
    </main>
